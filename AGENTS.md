# Villsource BuzMag .NET Starter Kit
...
| `src/Host/Villsource.BuzMag.Api` | Composition-root Web API host. |
| `src/Host/Villsource.BuzMag.AppHost` | .NET Aspire orchestrator (Postgres, Redis, MinIO, migrator, API, **both React apps**). |
| `src/Host/Villsource.BuzMag.DbMigrator` | One-shot migrate/seed runner. DB is **not** migrated at API startup. |
| `src/Host/Villsource.BuzMag.Migrations.PostgreSQL` | All EF migrations, organized per-module by folder. |
...
dotnet run --project src/Host/Villsource.BuzMag.AppHost   # one-time: npm install in clients/admin & clients/dashboard

dotnet build src/Villsource.BuzMag.slnx                   # build backend
dotnet run --project src/Host/Villsource.BuzMag.Api       # API only → https://localhost:7030 (/scalar)
dotnet test src/Villsource.BuzMag.slnx                    # tests — integration tests REQUIRE Docker
...
```bash
dotnet run --project src/Host/Villsource.BuzMag.DbMigrator -- apply [--seed]
dotnet run --project src/Host/Villsource.BuzMag.DbMigrator -- list-pending
```
