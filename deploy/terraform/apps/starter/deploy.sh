#!/usr/bin/env bash
#
# One-command deploy of the Villsource BuzMag Starter Kit to AWS.
#
...
if [[ "$BUILD_API" == true ]]; then
  ...
  dotnet publish "$REPO_ROOT/src/Host/Villsource.BuzMag.Api/Villsource.BuzMag.Api.csproj" \
  ...
  dotnet publish "$REPO_ROOT/src/Host/Villsource.BuzMag.DbMigrator/Villsource.BuzMag.DbMigrator.csproj" \
    -c Release -r linux-x64 \
    /t:PublishContainer \
    -p:ContainerRegistry="$REGISTRY_HOST" \
