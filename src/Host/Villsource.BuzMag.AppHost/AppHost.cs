...
// Per-app prefix from the AppHost assembly name (Villsource.BuzMag.AppHost -> fsh-starter); namespaces Docker volumes + resource names so multiple FSH apps don't clash.
...
var migrator = builder.AddProject<Projects.Villsource_BuzMag_DbMigrator>($"{appPrefix}-db-migrator")
    ...
    .WithEnvironment("DatabaseOptions__MigrationsAssembly", "Villsource.BuzMag.Migrations.PostgreSQL")
...
// API Service
var api = builder.AddProject<Projects.Villsource_BuzMag_Api>($"{appPrefix}-api")
    .WithReference(postgres)
    .WaitFor(postgres)
    .WaitFor(redis)
    ...
    .WithEnvironment("MailOptions__DisplayName", "Villsource BuzMag")
