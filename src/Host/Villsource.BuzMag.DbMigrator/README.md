...
dotnet run --project src/Host/Villsource.BuzMag.DbMigrator -- apply

# Apply only to one tenant.
dotnet run --project src/Host/Villsource.BuzMag.DbMigrator -- apply --tenant root

# Apply only the tenant catalog (no per-tenant pass).
dotnet run --project src/Host/Villsource.BuzMag.DbMigrator -- apply --catalog-only
...
dotnet run --project src/Host/Villsource.BuzMag.DbMigrator -- seed
...
DOTNET_ENVIRONMENT=Development \
  dotnet run --project src/Host/Villsource.BuzMag.DbMigrator -- seed-demo
```
...
as `Villsource.BuzMag.Api` (both files are linked into the project so they
...
| `DatabaseOptions__MigrationsAssembly`     | `Villsource.BuzMag.Migrations.PostgreSQL`         |
...
```bash
dotnet publish src/Host/Villsource.BuzMag.DbMigrator -c Release \
        ...
        - name: migrator
          ...
          env:
            ...
            - name: DatabaseOptions__MigrationsAssembly
              value: Villsource.BuzMag.Migrations.PostgreSQL
```
...
- **Aspire**: `dotnet run --project src/Host/Villsource.BuzMag.AppHost` —
...
- **Raw**: run the migrator once after pulling, before starting the
  ...
  # Then start the API
  dotnet run --project src/Host/Villsource.BuzMag.Api
  ```
