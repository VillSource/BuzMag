...
export function OrganizationUnitsPage() {
    ...
    const update = useMutation({
        mutationFn: ({referenceId, input}: {
            referenceId: string;
            input: OrganizationUnitInput
        }) => updateOrganizationUnit(referenceId, input),
        onSuccess: () => {
    ...
    const move = useMutation({
        mutationFn: ({referenceId, parentId}: {
            referenceId: string;
            parentId: string | null
        }) => moveOrganizationUnit(referenceId, parentId),
        onSuccess: () => {
            toast.success("Organization unit moved");
            setMoveOpen(false);
            refresh();
        },
    ...
    const submitForm = (event: FormEvent) => {
        ...
        if (editing) update.mutate({referenceId: editing.referenceId, input});
            ...
            ...input,
            parentId: parentForNew?.referenceId ?? null,
            organizationId: !!parentForNew ? null : selectedOrgId
        });
    };
    ...
    return (
        <div className="space-y-4 sm:space-y-6">
            ...
            {query.isLoading ? <EntityListLoading rows={6} desktopColumns={DESKTOP_COLUMNS}/>
                : tree.length === 0 ? <EntityEmpty icon={GitBranch} title="No organization units yet"
                                                   body="Create a root unit to start building your structure."
                                                   action={<Button onClick={() => openCreate()}><Plus
                                                       className="size-4"/>New root unit</Button>}/>
                    : <EntityListCard>
                        ...
                        {tree.map((unit, index) => <UnitRow key={unit.referenceId} unit={unit} depth={0}
                                                            isLast={index === tree.length - 1} expanded={expanded}
                                                            onToggle={toggle} onCreate={openCreate} onEdit={openEdit}
                                                            onMove={openMove} onDelete={(target) => {
                            if (window.confirm(`Delete ${target.name}? This cannot be undone.`)) remove.mutate(target.referenceId);
                        }} deleting={remove.isPending}/>)}
                    </EntityListCard>}

            {query.isError &&
                <p role="alert" className="text-sm text-[var(--color-destructive)]">The organization structure could not
                    be loaded.</p>}

            <UnitFormDialog open={formOpen} onOpenChange={setFormOpen} editing={editing} parent={parentForNew}
                            form={form} setForm={setForm} onSubmit={submitForm}
                            busy={create.isPending || update.isPending}/>
            <MoveUnitDialog open={moveOpen} onOpenChange={setMoveOpen} moving={moving} candidates={candidates}
                            parentId={moveParentId} setParentId={setMoveParentId} onSubmit={submitMove}
                            busy={move.isPending}/>
        </div>
    );
}

function UnitRow({unit, depth, isLast, expanded, onToggle, onCreate, onEdit, onMove, onDelete, deleting}: {
    unit: UnitNode;
    depth: number;
    isLast: boolean;
    expanded: Set<string>;
    onToggle: (referenceId: string) => void;
    onCreate: (unit: UnitNode) => void;
    onEdit: (unit: UnitNode) => void;
    onMove: (unit: UnitNode) => void;
    ...
    return <>
        <EntityListRow className={DESKTOP_COLUMNS} isLast={isLast}>
            <div className="flex min-w-0 items-center gap-2" style={{paddingLeft: `${depth * 20}px`}}>
                <button type="button"
                        className="grid size-7 shrink-0 place-items-center rounded hover:bg-[var(--color-accent)]"
                        onClick={() => hasChildren && onToggle(unit.referenceId)}
                        aria-label={hasChildren ? `${isExpanded ? "Collapse" : "Expand"} ${unit.name}` : undefined}>
                ...
                <div className="min-w-0">
                    <Link to={`/organizations/units/${unit.referenceId}`}
                          className="block truncate text-sm font-medium hover:text-[var(--color-primary)]">{unit.name}</Link>
                    <span
                        className="block truncate text-xs text-[var(--color-muted-foreground)]">{unit.description || "No description"}</span>
                </div>
            </div>
            <span className="font-mono text-xs text-[var(--color-muted-foreground)]">{unit.code}</span>
            <span
                className="truncate font-mono text-xs text-[var(--color-muted-foreground)]">{unit.referenceId || "—"}</span>
            <div className="flex items-center justify-end gap-1">
                <Button variant="ghost" size="icon-xs" onClick={() => onCreate(unit)}
                        aria-label={`Add child unit to ${unit.name}`} title="Add child unit"><Plus
                    className="size-3.5"/></Button>
                <Button variant="ghost" size="icon-xs" onClick={() => onEdit(unit)} aria-label={`Edit ${unit.name}`}
                        title="Edit"><Pencil className="size-3.5"/></Button>
                <Button variant="ghost" size="icon-xs" onClick={() => onMove(unit)} aria-label={`Move ${unit.name}`}
                        title="Move"><Move className="size-3.5"/></Button>
                <Button variant="ghost" size="icon-xs" disabled={deleting} onClick={() => onDelete(unit)}
                        aria-label={`Delete ${unit.name}`} title="Delete"><Trash2
                    className="size-3.5 text-[var(--color-destructive)]"/></Button>
            </div>
        </EntityListRow>
        {hasChildren && isExpanded && unit.children.map((child, index) => <UnitRow key={child.referenceId} unit={child}
                                                                                   depth={depth + 1}
                                                                                   isLast={isLast && index === unit.children.length - 1}
                                                                                   expanded={expanded}
                                                                                   onToggle={onToggle}
                                                                                   onCreate={onCreate} onEdit={onEdit}
                                                                                   onMove={onMove} onDelete={onDelete}
                                                                                   deleting={deleting}/>)}
    </>;
}

function UnitFormDialog({open, onOpenChange, editing, parent, form, setForm, onSubmit, busy}: {
    open: boolean;
    onOpenChange: (open: boolean) => void;
    editing: UnitNode | null;
    ...
    return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent>
        <form onSubmit={onSubmit}>
            <DialogHeader><DialogTitle>{editing ? "Edit organization unit" : parent ? `New unit under ${parent.name}` : "New root organization unit"}</DialogTitle><DialogDescription>{editing ? "Update the unit's identifying details." : "Name and code are required."}</DialogDescription></DialogHeader><DialogBody
            className="space-y-4">
            <div className="space-y-2"><Label htmlFor="unit-name">Name</Label><Input id="unit-name" value={form.name}
                                                                                     onChange={(event) => setForm((value) => ({
                                                                                         ...value,
                                                                                         name: event.target.value
                                                                                     }))} required autoFocus/></div>
            <div className="space-y-2"><Label htmlFor="unit-code">Code</Label><Input id="unit-code" value={form.code}
                                                                                     onChange={(event) => setForm((value) => ({
                                                                                         ...value,
                                                                                         code: event.target.value
                                                                                     }))} required/></div>
            <div className="space-y-2"><Label htmlFor="unit-description">Description</Label><textarea
                id="unit-description" value={form.description}
                onChange={(event) => setForm((value) => ({...value, description: event.target.value}))}
                className="min-h-24 w-full rounded-md border border-[var(--color-input)] bg-transparent px-3 py-2 text-sm"/>
            </div>
        </DialogBody><DialogFooter><DialogClose asChild><Button type="button"
                                                                variant="outline">Cancel</Button></DialogClose><Button
            type="submit"
            disabled={busy}>{busy ? "Saving…" : editing ? "Save changes" : "Create unit"}</Button></DialogFooter></form>
    </DialogContent></Dialog>;
}

function MoveUnitDialog({open, onOpenChange, moving, candidates, parentId, setParentId, onSubmit, busy}: {
    open: boolean;
    onOpenChange: (open: boolean) => void;
    moving: UnitNode | null;
    candidates: UnitNode[];
    parentId: string;
    ...
    return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent>
        <form onSubmit={onSubmit}><DialogHeader><DialogTitle>Move organization unit</DialogTitle><DialogDescription>Choose
            a new parent, or make {moving?.name ?? "this unit"} a root
            unit.</DialogDescription></DialogHeader><DialogBody><Label htmlFor="new-parent">New parent</Label><select
            id="new-parent" value={parentId} onChange={(event) => setParentId(event.target.value)}
            className="mt-2 h-9 w-full rounded-md border border-[var(--color-input)] bg-transparent px-3 text-sm">
            <option value="">Root organization unit</option>
            {candidates.map((unit) => <option key={unit.referenceId}
                                              value={unit.referenceId}>{unit.name} ({unit.code})</option>)}
        </select></DialogBody><DialogFooter><DialogClose asChild><Button type="button" variant="outline">Cancel</Button></DialogClose><Button
            type="submit" disabled={busy}>{busy ? "Moving…" : "Move unit"}</Button></DialogFooter></form>
    </DialogContent></Dialog>;
}
