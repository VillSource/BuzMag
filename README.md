...
```bash
dotnet run --project src/Host/Villsource.BuzMag.AppHost
```
...
```bash
dotnet run --project src/Host/Villsource.BuzMag.Api      # needs external Postgres + Redis
```
...
src/
  ...
  Host/
    Villsource.BuzMag.Api/                    API composition root
    Villsource.BuzMag.AppHost/                .NET Aspire orchestrator
    Villsource.BuzMag.DbMigrator/             One-shot migrate / seed runner
    Villsource.BuzMag.Migrations.PostgreSQL/  EF Core migrations
...
- [ ] **Mail** — configure SMTP / SendGrid under `MailOptions` in
      `src/Host/Villsource.BuzMag.Api/appsettings.json`.
...
```bash
dotnet test src/Villsource.BuzMag.slnx       # integration tests require Docker
```
